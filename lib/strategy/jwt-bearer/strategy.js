...
function Strategy(options, keying, verify) {
  ...
  if (!audience) {
    throw new TypeError(g.f('{{OAuth 2.0 JWT}} bearer strategy requires an audience option'));
  }
  if (!keying) {
    throw new TypeError(g.f('{{OAuth 2.0 JWT}} bearer strategy requires a keying callback'));
  }
  if (!verify) {
    throw new TypeError(g.f('{{OAuth 2.0 JWT}} bearer strategy requires a verify callback'));
  }
...
Strategy.prototype.authenticate = function (req) {
  ...
  if (req.body.grant_type !== jwt_grant_type && req.body.client_assertion_type !== jwt_assertion_type) {
    return this.fail();
  }

  const self = this;
  const assertion = (req.body.grant_type === jwt_grant_type) ? req.body.assertion
  ...
  const token = jwt.decode(assertion, { json: true });
  if (!token) {
    return this.fail(400);
  }
  ...
  function doKeyingStep() {
    function keyed(err, key) {
      ...
      const ok = jwt.verify(assertion, self.jwtAlgorithm, key);
      if (!ok) {
        return self.fail();
      }
      doVerifyStep();
