...
const MACGenerator = require('../../mac-token');
...
function Strategy(options, verify) {
  ...
  if (!verify) {
    throw new TypeError(g.f('{{MACStrategy}} requires a verify callback'));
  }
...
Strategy.prototype.authenticate = function (req) {
  ...
  const token = this._macGenerator.validate(req);

  if (!token) {
    return this.fail(this._challenge(g.f('Invalid {{MAC}} token')));
  }
