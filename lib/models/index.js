...
/*     updated: 08/07/20 17:42:15 by Guillaume Torresani                                */
/*                                                                                      */
/* ************************************************************************************ */
...
module.exports = function (app, options) {
  ...
  token.save = async function (token, clientId, resourceOwner, scopes, claims, refreshToken, connectionToken, done) {
    let tokenObj;
  ...
  code.save = async function (code, clientId, redirectURI, resourceOwner, scopes, claims, connectionToken, done) {
    let codeObj;
    if (arguments.length === 2 && typeof token === 'object') {
      // save(code, cb)
      codeObj = code;
  ...
  /*
     ...
     */
  permission.isAuthorized = function (appId, userId, scopes, claims, done) {
    permission.find(appId, userId, (err, perm) => {
      ...
      const ok = helpers.isScopeAuthorized(scopes, perm.scopes)
        && helpers.isClaimsAuthorized(claims, perm.claims);
  ...
  /*
     ...
     */
  permission.addPermission = function (appId, userId, scopes, claims, done) {
    oAuthPermissionModel.findOrCreate({
      ...
      perm.updateAttributes({ scopes: helpers.normalizeList(scopes), claims }, done);
    });
  };
