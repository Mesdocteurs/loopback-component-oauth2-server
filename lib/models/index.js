...
module.exports = function (app, options) {
  ...
  const getTTL = typeof options.getTTL === 'function' ? options.getTTL
  ...
  token.save = function (token, clientId, resourceOwner, scopes, refreshToken, connectionToken, done) {
    let tokenObj;
    ...
    const ttl = getTTL('token', clientId, resourceOwner, scopes);
    if (!tokenObj) {
      tokenObj = {
        id: token,
        appId: clientId,
  ...
  code.save = function (code, clientId, redirectURI, resourceOwner, scopes, connectionToken, done) {
    let codeObj;
    if (arguments.length === 2 && typeof token === 'object') {
      // save(code, cb)
      codeObj = code;
    ...
    const ttl = getTTL('code', clientId, resourceOwner, scopes);
    if (!codeObj) {
      codeObj = {
        id: code,
        appId: clientId,
