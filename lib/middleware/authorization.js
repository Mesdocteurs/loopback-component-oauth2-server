...
module.exports = function (server, options, validate, immediate) {
  ...
  if (!server) {
    throw new TypeError(g.f('{{oauth2orize.authorization}} middleware requires a {{server}} argument'));
  }
  if (!validate) {
    throw new TypeError(g.f('{{oauth2orize.authorization}} middleware requires a {{validate}} function'));
  }
  ...
  return function authorization(req, res, next) {
    if (!req.session) {
      return next(new Error(g.f('{{OAuth2orize}} requires {{session}} support.'
    ...
    server._parse(type, req, (err, areq) => {
      ...
      if (Object.keys(areq).length === 1 && areq.type) {
        return next(new AuthorizationError(g.f(
          'Unsupported response type: %s', type,
      ...
      function validated(err, client, redirectURI) {
        ...
        if (!client) {
          return next(new AuthorizationError(g.f('Invalid client'),
            'invalid_client'));
        }
        ...
        let validURIs = client.callbackUrls || client.redirectUris || client.redirectURIs || [];
        ...
        helpers.validateClient(client, { redirectURI }, next);
