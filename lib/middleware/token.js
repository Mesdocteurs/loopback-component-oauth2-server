...
module.exports = function token(server, options) {
  ...
  if (!server) { throw new TypeError(g.f('{{oauth2orize.token}} middleware requires a {{server}} argument')); }

  return function token(req, res, next) {
    const type = req.body.grant_type;

    server._exchange(type, req, res, (err) => {
      if (err) { return next(err); }
      return next(new TokenError(g.f('Unsupported grant type: %s', type), 'unsupported_grant_type'));
    });
  };
};
