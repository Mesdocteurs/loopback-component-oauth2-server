...
module.exports = function (options, issue) {
  ...
  if (!issue) { throw new TypeError(g.f('{{oauth2orize.refreshToken}} exchange requires an {{issue}} callback')); }
  ...
  return function refresh_token(req, res, next) {
    ...
    async function issued(err, accessToken, refreshToken, params, tokenInfo) {
      if (err) { return next(err); }
      if (!accessToken) { return next(new TokenError(g.f('Invalid refresh token'), 'invalid_grant')); }
      let id_token;
      if (params.models && params.models.openId && params.models.openId.generateOpenIdToken) {
        id_token = await params.models.openId.generateOpenIdToken(tokenInfo);
      }
    ...
    try {
      ...
      if (arity === 5) {
        issue(client, refreshToken, scope, req, issued);
      } else { // arity == 3
